...
class Analytical_crust_params(object):
    ...
    def set_constants(self,material,permeability):
        ...
        if (material =='Westerly_Granite'): #%
            ...
            self.c = permeability/self.S/self.visc #% m^2
    ...
    def Analytical_sol_cavity_T_Use(self,T_R,P_R,R_chamber,t) :
        try :
            tmp1 = t.shape[1]
            del tmp1
        except IndexError :
        ...
        A_3 = (term1_difft_R_chm*(self.c-self.c_a+self.C_Q) - 2.*term1_diffP_R_chm*self.alpha_p*self.c)*(self.c-self.c_a-self.C_Q)
        A_4 = (term1_difft_R_chm*(self.c-self.c_a-self.C_Q) - 2.*term1_diffP_R_chm*self.alpha_p*self.c)*(self.c-self.c_a+self.C_Q)
        ...
        term_T_der = -(self.term1/2./self.R_val)*(A_1*Erfc_t1a - A_2*Erfc_t1b) + \
                     (self.term1/2.)*(-2.*A_1*msc_fact1*exp_term1a + 2.*A_2*msc_fact2*exp_term1b)

        sqrt_time_new_term = sqrt_time_new/np.sqrt(pi)
        term_int1 = -((self.R_val + R_chamber)/self.lam1)*sqrt_time_new_term*exp_term1a + \
                    (0.5*tmp_one_t*(self.R_val**2. - R_chamber**2.) -self.tmp_one_R*time_new/self.lam1**2)*erfc(term1a)
        term_int2 = -((self.R_val + R_chamber)/self.lam2)*sqrt_time_new_term*exp_term1b + \
                    (0.5*tmp_one_t*(self.R_val**2. - R_chamber**2.) -self.tmp_one_R*time_new/self.lam2**2)*erfc(term1b)
