...
def Analytical_sol_cavity_T_Use(T_R,P_R,R_chamber,t,R_val,permeability,material) :
    ...
    A_1 = Diff_temp_arr*R_chamber*(c-c_a+C_Q) - 2.*Diff_press_arr*alpha_p*c*R_chamber
    A_2 = Diff_temp_arr*R_chamber*(c-c_a-C_Q) - 2.*Diff_press_arr*alpha_p*c*R_chamber
    A_3 = (Diff_temp_arr*R_chamber*(c-c_a+C_Q) - 2.*Diff_press_arr*alpha_p*c*R_chamber)*(c-c_a-C_Q)
    A_4 = (Diff_temp_arr*R_chamber*(c-c_a-C_Q) - 2.*Diff_press_arr*alpha_p*c*R_chamber)*(c-c_a+C_Q)

    term1 = (1./C_Q)/R_val
    term1a = lam1*(R_val - R_chamber)/2./np.sqrt(time_new.T)
    term1b = lam2*(R_val - R_chamber)/2./np.sqrt(time_new.T)
    term_T = (term1/2.)*( A_1*erfc(term1a) - A_2*erfc(term1b) )
    term_P = (term1/(4.*c*alpha_p))*( A_3*erfc(term1a) - A_4*erfc(term1b) )
