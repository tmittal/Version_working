...
class Chamber_Problem(Explicit_Problem):
    ...
    def __init__(self,depth=1e3,chamber_shape='spherical',**kwargs):
        ...
        self.param['frac_length'] = 0.1
        self.flux_in_vol = np.array([1e-7])
        self.flux_out_vol = np.array([1e-7])
    ...
    def rhs(self,t,y,sw) :
        ...
        if eruption == 0:
            if self.param['vol_degass'] == 1.:
                ...
                U_og = md_deg.func_Uog(eps_g,eps_x,m_eq,rho_m,rho_g,T,delta_P_grad,r_b = 10*1e-6)
                ...
                degass_hdot_water2 = self.param['c_g']*T*Mdot_out2
        ...
        if self.param['press_relax'] ==1 :
            P_loss2 = np.tanh(eps_g*40.)*(self.permeability/visc_gas)*(P - mean_P_out)/(self.param['frac_rad_press']*a)**2. # Set that the P_loss2 is only when eps_g > 0.05
        ...
        if t > self.tcurrent :
            ...
            if (eruption ==0) and (self.dt_counter/self.dt_init > 0.95) :
                #print(self.dt/(365.*24.*3600.),sss )
                self.P_list = np.hstack([self.P_list,P-self.plith])
                self.T_list = np.hstack([self.T_list,T-self.param['T_S']])
                ...
                self.times_list = np.hstack([self.times_list,t])
                #pdb.set_trace()
                self.T_out1,self.P_out1,self.sigma_rr,self.sigma_theta,self.T_der = Analytical_sol_cavity_T_Use(self.T_list,self.P_list,self.radius,self.times_list,self.R_outside,self.permeability,self.param['material'])
                ...
                #print(self.times_list[-3:],np.diff(self.times_list)[-3:])
                self.T_out = self.T_out1 + self.T_out_p2
                self.P_out = self.P_out1 + self.P_out_p2
                self.dt_counter = 0.
                #pdb.set_trace()
                ...
                self.flux_in_vol = np.hstack([self.flux_in_vol,Mdot_v_in])
                self.flux_out_vol = np.hstack([self.flux_out_vol,Mdot_v_out])
            if (eruption ==1)  and (self.dt_counter/self.dt_init > 0.001):
                self.P_list = np.hstack([self.P_list,P-self.plith])
                self.T_list = np.hstack([self.T_list,T-self.param['T_S']])
                self.P_flux_list = np.hstack([self.P_flux_list,0])
                self.T_flux_list = np.hstack([self.T_flux_list,0])
                self.times_list = np.hstack([self.times_list,t])
                self.T_out,self.P_out,self.sigma_rr,self.sigma_theta,self.T_der = Analytical_sol_cavity_T_Use(self.T_list,self.P_list,self.radius,self.times_list,self.R_outside,self.permeability,self.param['material'])
                #print(self.times_list[-3:],np.diff(self.times_list)[-3:])
                self.dt_counter = 0.
                #pdb.set_trace()
                self.P_out_all = np.vstack([self.P_out_all,self.P_out])
